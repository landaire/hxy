//! Bytecode VM counterpart to `probe_one`. Runs a single template
//! through `Interpreter::run_bytecode_experimental` so xct2cli can
//! sample where the bytecode path actually spends its time.
//!
//! Usage: `probe_bc <template.hexpat> [fixture]`. Step limit is
//! pumped to 100M so the full bencode walk completes.
...
fn main() {
    let path = env::args().nth(1).expect("usage: probe_bc <template> [fixture]");
