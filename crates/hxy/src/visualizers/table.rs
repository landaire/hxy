...
pub fn show(ui: &mut egui::Ui, ctx: &VisualizerContext) {
    ...
    let parent_idx = ctx.tree.nodes.iter().position(|n| std::ptr::eq(n, ctx.node)).map(|i| i as u32);
    let Some(parent_idx) = parent_idx else {
        ui.weak(hxy_i18n::t("visualizer-table-no-children"));
        return;
    };
    let children: Vec<&Node> = ctx.tree.nodes.iter().filter(|n| n.parent == Some(parent_idx)).collect();
    if children.is_empty() {
        ui.weak(hxy_i18n::t("visualizer-table-no-children"));
        return;
    }
    ui.label(
        egui::RichText::new(hxy_i18n::t_args("visualizer-table-info", &[("count", &children.len().to_string())]))
            .weak(),
    );
    ui.add_space(4.0);

    egui::ScrollArea::both().auto_shrink([false, false]).show(ui, |ui| {
        egui::Grid::new(ctx.ui_id.with("table")).striped(true).num_columns(5).show(ui, |ui| {
            ...
            for child in &children {
                ...
                ui.label(
                    format_value_for_table(child, ctx.template_value_formats, ctx.inverse_format).unwrap_or_default(),
                );
                ui.end_row();
            }
        });
