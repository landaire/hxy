//! Native desktop `HxyApp`. The wasm browser build's `HxyApp`
//! lives in [`super::wasm`]; both are re-exported as `HxyApp`
//! from `lib.rs` so call sites refer to one symbolic type.
//! Future commits keep migrating common rendering / dispatch
//! into shared helpers under `super::common` so the two impls
//! converge.
